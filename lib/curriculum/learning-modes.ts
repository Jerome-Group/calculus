...
export const pilotLessons: Record<string, PilotLesson> = {
  "epsilon-delta-one-variable": {
    ...
    learn: {
      ...
      theorem:
        "For every $\\varepsilon>0$ there exists $\\delta>0$ such that every domain point with $0<|x-a|<\\delta$ satisfies $|f(x)-L|<\\varepsilon$. Delta may depend on epsilon and a, never on the later input x.",
      reasoning:
      ...
      quickCheck:
        "If $\\varepsilon=0.4$ for $f(x)=2x$ at 1, what delta works? Check it in Explore, then prove the implication for every input.",
    },
    explore: {
      prediction:
        "Before moving the tolerance control, predict what happens to delta when epsilon is halved.",
      control:
        "Output tolerance epsilon; the linear example derives delta = epsilon/2.",
      invariant:
    ...
    practice: [
      {
        ...
        choices: [
          {
            id: "correct",
            text: "For every epsilon, choose one delta; then every nearby x obeys the bound.",
          },
          {
            id: "reversed",
          ...
          {
            id: "sample",
            text: "Check a few x values for each epsilon.",
        ...
        explanation:
          "Epsilon is chosen first; one delta must work for all later admissible inputs.",
        feedback: {
          QUANTIFIER_REVERSED: "Delta cannot depend on the later input x.",
          SAMPLE_AS_PROOF:
      ...
      },
      {
        ...
        kind: "recognition",
        prompt:
          "Which proposed delta is invalid because it depends on the input chosen after epsilon?",
        choices: [
        ...
        feedback: {
          QUANTIFIER_REVERSED:
            "This expression depends only on epsilon, not on the later input x.",
        },
      },
    ],
    revise: {
